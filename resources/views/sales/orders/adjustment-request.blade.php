...
@section('title', __('طلب تعديل') . ' | WorkFlow')
...
@section('content')
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <div>
            <h1 class="h3 mb-1">{{ __('طلب تعديل للطلب') }} {{ $order->order_number }}</h1>
            <div class="text-muted">{{ __('سيتم إرسال التعديلات للمدير للمراجعة والاعتماد قبل تثبيتها على السجل الأصلي.') }}</div>
        </div>
        <div class="d-flex align-items-center gap-2">
            <span class="badge-status status-{{ $order->status }}">{{ $order->status_label }}</span>
            <a href="{{ route('sales.orders.edit', $order) }}" class="btn btn-outline-secondary">{{ __('رجوع') }}</a>
        </div>
    </div>

    <div class="alert alert-info border-0 shadow-sm">
        {{ __('الحقول الأصلية مقفلة لأن الطلب دخل المسار التشغيلي. عبّئ النموذج التالي وسيبقى الطلب الأصلي دون تعديل حتى اعتماد الإدارة.') }}
    </div>
    ...
    <form method="POST" action="{{ route('sales.orders.adjustments.store', $order) }}" enctype="multipart/form-data" class="row g-4">
        ...
        <div class="col-12">
            <div class="card form-card">
                <div class="card-body p-4">
                    <h2 class="h5 section-title">{{ __('بيانات العميل المقترحة') }}</h2>
                    <div class="row g-3">
                        <div class="col-md-6">
            ...
            <div class="card form-card">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                        <h2 class="h5 section-title mb-0">{{ __('العناصر المقترحة') }}</h2>
        ...
        <div class="col-lg-6">
            <div class="card form-card h-100">
                <div class="card-body p-4">
                    <h2 class="h5 section-title">{{ __('مرفقات إضافية مع طلب التعديل') }}</h2>
        ...
        <div class="col-lg-6">
            <div class="card form-card h-100">
                <div class="card-body p-4">
                    <h2 class="h5 section-title">{{ __('المرفقات الحالية') }}</h2>
        ...
        <div class="col-12 d-grid gap-2">
            <button type="submit" class="btn btn-primary btn-lg">{{ __('إرسال طلب التعديل للاعتماد') }}</button>
        </div>
    </form>
@endsection
