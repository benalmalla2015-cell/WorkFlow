...
@section('title', __('طلب تعديل مصنع') . ' | WorkFlow')

@section('content')
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <div>
            <h1 class="h3 mb-1">{{ __('طلب تعديل تشغيلي للطلب') }} {{ $order->order_number }}</h1>
            <div class="text-muted">{{ __('سيُرسل التعديل إلى الإدارة لاعتماده قبل تحديث بيانات المصنع الأصلية.') }}</div>
        </div>
        <div class="d-flex align-items-center gap-2">
            <span class="badge-status status-{{ $order->status }}">{{ $order->status_label }}</span>
            <a href="{{ route('factory.orders.edit', $order) }}" class="btn btn-outline-secondary">{{ __('رجوع') }}</a>
        </div>
    </div>

    <div class="alert alert-info border-0 shadow-sm">
        {{ __('استخدم هذا النموذج فقط عندما تحتاج تعديل بيانات المورد أو التكلفة أو مدة الإنتاج بعد انتهاء الإدخال الأولي وإرسال الطلب للإدارة.') }}
    </div>
    ...
    <div class="row g-4 mb-4">
        <div class="col-lg-5">
            <div class="card form-card mb-4">
                <div class="card-body p-4">
                    <h2 class="h5 section-title">{{ __('ملخص الطلب') }}</h2>
                    <dl class="row mb-0">
            ...
            <div class="card form-card">
                <div class="card-body p-4">
                    <h2 class="h5 section-title">{{ __('مرفقات مرجعية') }}</h2>
        ...
        <div class="col-lg-7">
            <form method="POST" action="{{ route('factory.orders.adjustments.store', $order) }}" enctype="multipart/form-data">
                ...
                <div class="card form-card mb-4">
                    <div class="card-body p-4">
                        <h2 class="h5 section-title">{{ __('مرفقات داعمة') }}</h2>
                ...
                <button type="submit" class="btn btn-primary btn-lg w-100">{{ __('إرسال طلب التعديل للاعتماد') }}</button>
            </form>
        </div>
    </div>
@endsection
